...
export function residualDomainInstrumentation():Plugin{
 return {name:'residual-observe-current-domains',enforce:'pre',transform(source,id){
  ...
  if(id.endsWith('/generateCrossword.ts')){replace('      const placements = constructCrossword(', '      beginDomainAttempt(template,prepared,candidateWindows);\n      const placements = constructCrossword(');replace('      if (!placements.length) {','      finishDomainAttempt(placements.length);\n      if (!placements.length) {');}
  ...
  return {code:`import { observeDomain,beginDomainAttempt,finishDomainAttempt } from ${JSON.stringify(fileURLToPath(new URL('./residual-domain-state.ts',import.meta.url)))};\n${code}`,map:null};
 }};
}
