// Experimental layout rules only; never imported by production.
export let blockRatio = 0.3;
export let allowUnchecked = false;
export let maxBlocksInRun = 2;
export function configure(options: { density: number; unchecked?: boolean; blockRun?: number }) {
  blockRatio = options.density; allowUnchecked = options.unchecked ?? false; maxBlocksInRun = options.blockRun ?? 2;
}
export function setBlockRatio(value: number) { blockRatio = value; }
...
function validateBlockRunsLocal(grid: number[][], minRunLength = 2): boolean {
  ...
  if (allowUnchecked) for (let r=0;r<size;r++) for(let c=0;c<size;c++) {
    if (!grid[r][c]) continue;
    let horizontal=1,vertical=1;
    for(let x=c-1;x>=0&&grid[r][x];x--)horizontal++;
    for(let x=c+1;x<size&&grid[r][x];x++)horizontal++;
    for(let y=r-1;y>=0&&grid[y][c];y--)vertical++;
    for(let y=r+1;y<size&&grid[y][c];y++)vertical++;
...
function validateBlockRunsBlocks(grid: number[][]): boolean {
  ...
  for (let r = 0; r < size; r++) {
    ...
    for (let c = 0; c <= size; c++) {
      ...
      if (isBlock) run++;
      if (!isBlock || c === size) {
        if (run > maxBlocksInRun) return false;
        run = 0;
      }
    }
  }
...
export function getNYTTemplate(size: number, minRunLength = 2): number[][] {
  ...
  const maxBlocks = Math.floor(size * size * blockRatio);
  let blocks = 0;
...
export function getTemplate(size: number, minRunLength = 2): number[][] {
  ...
  for (let i = 0; i < attempts && blocks < maxBlocks; i++) {
    ...
    if (!allowUnchecked && !canConvertToBlack(grid, r, c)) continue;
    grid[r][c] = 0;
