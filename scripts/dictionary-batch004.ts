...
import { DICTIONARY_BATCH003_QA as source } from '../api/_lib/dictionary.stage3b.batch003.qa.generated.ts';
...
import { extractBatch004Patterns,matchesPattern,selectBatch004,sampleBatch004Qa,compileBatch004QaNotes,BATCH004_QA_SEED } from './stage3b-batch004.ts';
...
const root=new URL('../',import.meta.url),out=new URL('dictionary/stage3b/batch004/',root);
...
for(const [file,pinned]of Object.entries(inputs))if(hash(read(file))!==pinned)throw new Error(`Batch004 immutable input drift ${file}`);
const prior=new Set(['dictionary/stage3a/decisions.json','dictionary/stage3b/batch001/decisions.json','dictionary/stage3b/batch002/decisions.json','dictionary/stage3b/batch003/decisions.json'].flatMap(f=>(JSON.parse(read(f)) as {english:string}[]).map(d=>d.english)));
const reference=parseReferenceCsv(read('api/english_arabic_10000_v3.csv'));
if(JSON.stringify(extractBatch004Patterns(JSON.parse(read('generator_failure_analysis/diagnostic_runs.json'))))!==JSON.stringify(JSON.parse(readOut('diagnostic_patterns.json'))))throw new Error('Diagnostic evidence drift');
const selection=selectBatch004(source,reference,prior,JSON.parse(readOut('deferred.json')),JSON.parse(readOut('diagnostic_patterns.json')));
if(JSON.stringify(selection)!==JSON.stringify(JSON.parse(readOut('selection.json'))))throw new Error('Batch004 selection drift');
const compiled=compileBatch003Notes(source,selection,readOut('review-notes.txt'),JSON.parse(readOut('review-details.json')));
if(JSON.stringify(compiled)!==JSON.stringify(JSON.parse(readOut('proposals.json'))))throw new Error('Batch004 explicit-note proposal drift');
const initial=applyBatch003(source,selection,compiled,JSON.parse(readOut('rejection_checks.json'))),sample=sampleBatch004Qa(initial.decisions,reference,selection);
if(JSON.stringify(sample)!==JSON.stringify(JSON.parse(readOut('qa/sample.json'))))throw new Error('Batch004 QA sample drift');
if(JSON.stringify(initial.decisions)!==JSON.stringify(JSON.parse(readOut('decisions.json'))))throw new Error('Batch004 post-rejection decision drift');
if(JSON.stringify(compileBatch004QaNotes(sample,readOut('qa/review-notes.txt')))!==JSON.stringify(JSON.parse(readOut('qa/reviews.json'))))throw new Error('Batch004 explicit QA note drift');
...
const qaSummary={seed:BATCH004_QA_SEED,sampleSize:sample.length,headwords:new Set(sample.map(s=>s.english)).size,strata:group(sample.map(s=>s.stratum)),
 ...
 assessment:'Bounded same-assistant QA with controlled relationship-specific POS and short senses. See measured agreement and all disagreements; no independent-human certification. Batch005 has not begun.',
...
const summary={batch:'004',selectionMethod:'500 new review headwords. Score = reference membership 100 + 80*(1-originalIndex/16018) + English length bonus (3:18; 7/9/11/13:10) + Arabic 2-4 bonus 8 + min(12,3*English zero-pattern matches) + min(6,2*Arabic zero-pattern matches). Prior stages and frozen name/brand deferrals excluded. Tie: original index. Learner quality is manually reviewed, never inferred from score.',
 ...
 qa:qaSummary,batch005Started:false,
};
const distributionRows=[1,2,3,4].flatMap(batch=>{
...
const patternCoverage=JSON.parse(readOut('diagnostic_patterns.json')) as {mode:string;pattern:string}[];
const normalized=(s:string)=>normalizeArabicWord(s).replace(/[أإآٱ]/g,'ا');
const beforeIndex=createCandidateIndex(source,'approved-only'),afterIndex=createCandidateIndex(qa.dictionary,'approved-only');
const patternCounts=(index:ReturnType<typeof createCandidateIndex>,p:{mode:string;pattern:string})=>new Set([...index.get(`13:${p.mode}:advanced`)!.values()].flat(2).map(pair=>normalized(pair.answer)).filter(a=>matchesPattern(a,p.pattern))).size;
const patternRows=patternCoverage.map(p=>({...p,beforeUniqueAnswers:patternCounts(beforeIndex,p),afterUniqueAnswers:patternCounts(afterIndex,p),scope:'13-cell indexed pool before sampling; ignores used-word constraints'}));
// Small typed arrays avoid quadratic subtype comparisons across 16,018 object literals.
// Every chunk remains fully checked; concatenation preserves original row order.
const generated=(name:string,data:readonly DictionaryHeadword[])=>{
 const chunks=Array.from({length:Math.ceil(data.length/100)},(_,i)=>`const chunk${i}: DictionaryHeadword[] = ${JSON.stringify(data.slice(i*100,(i+1)*100),null,2)};\n`);
 return '// Generated by npm run dictionary:batch004. Preserves all prior source layers; do not edit.\n'+"import type { DictionaryHeadword } from './dictionary.js';\n"+chunks.join('')+`export const ${name}: DictionaryHeadword[] = [${chunks.map((_,i)=>`...chunk${i}`).join(',')}];\n`;
};
const outputs=new Map<URL,string>([
 [new URL('api/_lib/dictionary.stage3b.batch004.generated.ts',root),generated('DICTIONARY_BATCH004',initial.dictionary)],
 [new URL('api/_lib/dictionary.stage3b.batch004.qa.generated.ts',root),generated('DICTIONARY_BATCH004_QA',qa.dictionary)],
 ...
 [new URL('diagnostic_pattern_coverage.csv',out),csv(patternRows)],
...
else if(process.argv[2]==='audit'){for(const [url,text]of outputs)if(readFileSync(url,'utf8')!==text)throw new Error(`Batch004 output drift ${url.pathname}`);}
else throw new Error('Use generate or audit');
console.info(JSON.stringify({batch:'004',before:summary.before,after:summary.afterQa,batchAfterQa:summary.batchAfterQa,referenceConflicts:summary.referenceConflicts,qa:qaSummary},null,2));
